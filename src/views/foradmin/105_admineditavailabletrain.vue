<template>
  ...
  <div style="margin-top: 5%;">
    <strong style="margin-top: 10%;font-size: 200%;">
      <center>ADD AVAILABLE TRAIN </center>
    </strong>
    ...
    <v-sheet width="400" class="mx-auto sheet">
      ...
      <v-form @submit="form_submit" style="background-color: black; color: yellowgreen;">
        <!-- <v-text-field name="train_no" v-model="train_no" label="Train Number"></v-text-field> -->

        <v-select name="train_no" v-model="train_no" label=" train_no" :items="items" ></v-select>
        
        <v-text-field name="avaliable_train_name" v-model="avaliable_train_name" label="Name"></v-text-field>

        <v-select name="avaliable_train_type" v-model="avaliable_train_type" :items="['Air', 'Fan']" label="Type" required></v-select>
      
        <v-select name="avaliable_train_class" v-model="avaliable_train_class" :items="['1', '2', '3']" label="Class" required></v-select>

        <v-text-field name="avaliable_train_price" v-model="avaliable_train_price" label="Price"></v-text-field>
...
export default {
  ...
  data() {
    return {
      ...
      status: 'กำลังโหลด',
      items: [],
      avaliable_train_name:null,
      avaliable_train_type:null,
      avaliable_train_class:null,
      avaliable_train_price:null,
      train_no: null,
  ...
  mounted() {   //โหลดข้อมูลทันทีเมื่อแสดงคอมโพเนนต์
    let index = 0;
    fetch('/api/db/read/train_for105')
      ...
      .then(result => {
        if (result.length > 0) {
          ...
          while (result.length != index) {
            this.items[index] = result[index].train_no
            index++;
          }
        } else {
          this.docs = []
